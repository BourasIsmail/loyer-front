...
import {
  getProprietaires,
  generateConfirmedPaymentsReport,
} from "../api/proprietaire";
...
export default function Home() {
  ...
  const generatePdfMutation = useMutation(
    ({ id, year }: { id: number; year: number }) =>
      generateConfirmedPaymentsReport(id, year),
    {
      onSuccess: (data) => {
        ...
        link.setAttribute(
          "download",
          `confirmed_payments_${selectedProprietaires[0]?.nomComplet}_${selectedYear}.pdf`
        );
          ...
          <div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button disabled={selectedProprietaires.length === 0}>
                  Générer Rapport PDF
                </Button>
              </DialogTrigger>
              <DialogContent>
